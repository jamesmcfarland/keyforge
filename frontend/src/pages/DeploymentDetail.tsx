...
import { NewSocietyModal } from '../components/NewSocietyModal'
    ...
    <div className="container mx-auto p-6">
      <div className="bg-white shadow rounded-lg p-6 mb-6">
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-3xl font-bold">{deployment.union.name}</h1>
          {isFetching > 0 && (
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <p className="font-mono text-sm">{deployment.union.id}</p>
          </div>
          <div>
            <p className="text-sm text-gray-500">Status</p>
            <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800">
              {deployment.union.status}
            </span>
          </div>
          <div>
            <p className="text-sm text-gray-500">Created</p>
            <p className="text-sm">{format(deployment.union.created_at, 'PPpp')}</p>
          </div>
          <div>
            <p className="text-sm text-gray-500">VaultWarden URL</p>
            <p className="text-sm font-mono">{deployment.union.vaultwd_url}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white shadow rounded-lg p-6 mb-6">
        ...
        {deployment.societies.length === 0 ? (
          <p className="text-gray-500 text-sm">No societies created yet</p>
        ) : (
          <div className="space-y-3">
            {deployment.societies.map((society) => (
              <div key={society.id} className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50">
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <h3 className="font-semibold text-lg">{society.name}</h3>
                    <p className="text-xs text-gray-500 font-mono mt-1">{society.id}</p>
                  </div>
                  <div className="flex items-center space-x-3">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                      society.status === 'created' ? 'bg-green-100 text-green-800' : 
                      society.status === 'pending' ? 'bg-yellow-100 text-yellow-800' : 
                      'bg-red-100 text-red-800'
                    }`}>
                      {society.status}
                    </span>
                    {society.status === 'created' && (
                      <Link
                        to={`/deployments/${deployment.union.id}/societies/${society.id}`}
                ...
                {society.vaultwd_org_id && (
                  <p className="text-xs text-gray-600 mt-2">
                    VaultWarden Org: <span className="font-mono">{society.vaultwd_org_id}</span>
                  </p>
                )}
              </div>
            ))}
